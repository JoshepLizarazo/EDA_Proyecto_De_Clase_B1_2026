...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Mecanismo propio de actualizaci\'on de pesos:} ideamos un sistema de eventos epidemiol\'ogicos autom\'aticos por umbral (ALERTA\_LEVE, CUARENTENA, LOCKDOWN) que modifican din\'amicamente todos los pesos del grafo durante la ejecuci\'on.
...
Representa una arista dirigida y ponderada. El peso (\texttt{probContagio}) se calcula al crearla y est\'a clampado a $[0.05, 0.95]$ tanto en el constructor como en el setter. Adem\'as, expone el m\'etodo \texttt{aplicarFactor(double)} usado por el sistema de eventos para modificar din\'amicamente la probabilidad de contagio.
...
public class Contacto {
    ...
    private double probContagio;  // peso clampado a [0.05, 0.95]

    public Contacto(Persona origen, Persona destino, double prob) {
        this.origen = origen;
        this.destino = destino;
        this.probContagio = clamp(prob, 0.05, 0.95);
    }
...
Representa un evento de salud p\'ublica que modifica el grafo cuando se cruza un umbral de infectados. Incluye una bandera \texttt{yaDisparado} para evitar reactivaciones. Se desarrolla en detalle en la secci\'on~\ref{sec:eventos}.
...
La r\'ubrica del proyecto exige expl\'icitamente que cada equipo dise\~ne ``\textit{un mecanismo propio para actualizar informaci\'on en medio de la ejecuci\'on, como pesos de aristas}''. Nuestra implementaci\'on son los \textbf{eventos epidemiol\'ogicos por umbral}.

\subsection{Concepto: intervenciones no farmac\'euticas}

En una epidemia real, los gobiernos reaccionan cuando la cantidad de infectados supera ciertos umbrales: primero recomiendan medidas b\'asicas, luego restringen la movilidad y finalmente confinan a la poblaci\'on. Cada una de estas medidas reduce la probabilidad de que una persona contagie a otra. A estas medidas se les llama \textit{Intervenciones No Farmac\'euticas} (NPI por sus siglas en ingl\'es).

En nuestro c\'odigo modelamos esto de forma directa: \textbf{cuando se supera un umbral, multiplicamos el peso de todas las aristas del grafo por un factor reductor}. La red social ``se encoge'' din\'amicamente, reflejando que la gente se a\'isla, usa tapabocas y evita lugares concurridos.

\subsection{Los tres eventos del sistema}

\begin{table}[H]
\centering
\caption{Eventos epidemiol\'ogicos autom\'aticos.}
\label{tab:eventos}
...
Cada evento se dispara \textbf{exactamente una vez} por simulaci\'on. El \textit{flag} \texttt{yaDisparado} dentro de \texttt{EventoEpidemiologico} garantiza que aunque el umbral siga superado en turnos posteriores, el evento no se vuelva a aplicar.

\subsection{Formalizaci\'on matem\'atica}

Matem\'aticamente, los eventos representan una perturbaci\'on externa aplicada al conjunto de pesos $w$ del grafo en el turno $t^*$ en que se cruza el umbral $\theta$:
\begin{equation}
\forall (u,v) \in E : \quad w'(u,v) = w(u,v) \cdot \lambda_{\text{evento}} \quad \text{si} \quad \frac{I(t^*)}{N} \geq \theta
\end{equation}
donde $\lambda_{\text{evento}} \in (0, 1)$ es el factor de atenuaci\'on del evento.

\subsection{Algoritmo del \texttt{GestorEventos}}

\begin{algorithm}[H]
\caption{\texttt{GestorEventos.evaluar} -- ejecutado al final de cada turno}
\begin{algorithmic}[1]
\Require Red social $G$, turno actual $t$, lista de eventos $\mathcal{E}$
\State infectados $\gets$ contar nodos en estado INFECTADO
\State porcentaje $\gets$ infectados / $|V|$
\ForAll{evento $e \in \mathcal{E}$ en orden ALERTA\_LEVE $\rightarrow$ CUARENTENA $\rightarrow$ LOCKDOWN}
    \If{$\neg e.\text{yaDisparado}$ \textbf{y} porcentaje $\geq e.\text{umbral}$}
        \ForAll{arista $c \in G.\text{getTodosLosContactos}()$}
            \State $c.\text{aplicarFactor}(e.\text{factorMultiplicador})$
        \EndFor
        \State $e.\text{disparar}()$ \Comment{marca como ya disparado}
    \EndIf
\EndFor
\end{algorithmic}
\end{algorithm}

\subsection{Efecto acumulativo: ejemplo num\'erico}

Si en la misma simulaci\'on se disparan los tres eventos, cada arista acumula los tres factores. Consideremos una arista con \texttt{probContagio} original $= 0.50$:

\begin{table}[H]
\centering
\caption{Efecto acumulativo de los tres eventos sobre una arista.}
\label{tab:efecto_acumulado}
\begin{tabular}{c l c c}
\toprule
\textbf{Turno} & \textbf{Evento disparado} & \textbf{C\'alculo} & \textbf{Peso resultante} \\
\midrule
12 & \texttt{ALERTA\_LEVE} (30\%) & $0.50 \times 0.80$ & $0.40$ \\
18 & \texttt{CUARENTENA} (50\%) & $0.40 \times 0.50$ & $0.20$ \\
24 & \texttt{LOCKDOWN} (70\%) & $0.20 \times 0.20$ & $0.04$ \\
\bottomrule
\end{tabular}
\end{table}

La arista que originalmente ten\'ia 50\% de probabilidad de contagio por turno termina con apenas 4\%: el contagio se redujo al 8\% del original. Matem\'aticamente:
\[
w_{\text{final}} = w_{\text{original}} \cdot 0.80 \cdot 0.50 \cdot 0.20 = w_{\text{original}} \cdot 0.08
\]
...
\begin{enumerate}[leftmargin=*]
    ...
    \begin{enumerate}
        ...
        \item \texttt{GestorEventos.evaluar(red, turno)}: revisa umbrales y modifica los pesos de las aristas si corresponde.
        \item \texttt{GraficoSimulacion.actualizarTurno(\ldots)}: actualiza colores en pantalla y la barra de turno SIRV.
    \end{enumerate}
    \item Al final, \texttt{CalculadorEstadisticas} calcula las m\'etricas y \texttt{AnalisisComparativo} rankea las estrategias.
    \item \texttt{VentanaResultados} muestra tabla, ranking y curva $I(t)$.
...
\textsc{Creaci\'on de nodo} & Permite ver la recreaci\'on de la red nodo a nodo, desde un grafo vac\'io hasta la red completa. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item Actualiza el grosor de las aristas cuando \texttt{GestorEventos} modifica los pesos.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{La estrategia H\'ibrida (aporte propio) result\'o la m\'as efectiva de forma consistente} a lo largo del experimento por lotes: al combinar posici\'on estructural (intermediaci\'on topol\'ogica y grado) con perfil demogr\'afico individual (edad y estrato), logr\'o mayor contenci\'on que \textit{Betweenness} y \textit{Hubs} de forma aislada, confirmando las hip\'otesis H3 y H7.

    % ============================================================
    % [COMPLETAR antes de la entrega final con datos del PDF:]
    % - Pico de infectados de H\'ibrida vs. las dem\'as estrategias
    % - Porcentaje de contenci\'on final de la estrategia ganadora
    % - Score compuesto del ganador y del segundo lugar
    % - N\'umero de victorias de H\'ibrida en las 10.000 corridas
    % ============================================================

    \item \textbf{El modo por lotes (10\,000 grafos por estrategia, 60\,000 simulaciones en total) valida la robustez estad\'istica del veredicto.} Con una muestra de esta magnitud, el ranking final no es producto de una semilla favorable: la estrategia ganadora lo es porque domina de forma consistente sobre toda la distribuci\'on de topolog\'ias posibles.

    % ============================================================
    % [COMPLETAR antes de la entrega final con datos del PDF:]
    % - Victorias acumuladas de cada estrategia (ej: H\'ibrida: X, Betweenness: Y...)
...
\begin{itemize}[leftmargin=*]
    ...
    \item Dejamos los factores de los eventos NPI (0.80, 0.50, 0.20) est\'aticos. En la realidad, la efectividad de las medidas depende del cumplimiento y de la fatiga social.
...
\begin{enumerate}
    \item Seleccionar modo: \textbf{individual}, \textbf{comparativo} o \textbf{experimento por lotes}.
    \item Configurar par\'ametros: tama\~no de la red ($N = 300$ o personalizado), estrategia (solo en individual), n\'umero de grafos (solo en lotes -- por defecto 10\,000), turnos m\'aximos, d\'ias de recuperaci\'on.
